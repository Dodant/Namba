...
export const KO: Messages = {
  ...
  home: {
    ...
    empty: '아직 작성된 항목이 없습니다.', entryKinds: '항목 형식', categories: '카테고리', all: '전체',
  ...
  browse: {
    ...
    summary: (n: number, section: Section) =>
      `${({ number: '이 숫자를', abbr: '이 줄임말을', calendar: '이 날짜를' })[section]} 설명하는 항목이 ${fmtCount(n, 'ko')}개 있습니다.`,
  ...
  form: {
    ...
    guidelines: '항목 작성 지침', fixedValue: (noun: string) => `고정됨 — 다른 ${noun}에는 별도 항목이 필요`,
